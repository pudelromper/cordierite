...
# Install flatpaks
# Cordierite does not embed Flatpaks in the ISO by default (keeps the ISO and
# the build's storage needs small); bazzite-flatpak-manager installs the default
# set on first boot instead. Build with EMBED_FLATPAKS=1 to restore embedding.
curl --retry 3 -Lo /etc/flatpak/remotes.d/flathub.flatpakrepo https://dl.flathub.org/repo/flathub.flatpakrepo
if [[ "${EMBED_FLATPAKS:-0}" == "1" ]]; then
