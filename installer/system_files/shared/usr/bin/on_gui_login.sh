...
if [[ ! -d /sys/firmware/efi ]]; then
    legacy_boot_message="Cordierite har inte stöd för CSM/Legacy-start. Starta i inställningarna för UEFI/BIOS, inaktivera CSM/Legacy-läge och starta sedan om."
    shutdown_button="Stäng av"
    if is_swedish_locale; then
        legacy_boot_message="Cordierite har inte stöd för CSM/Legacy-start. Starta i inställningarna för UEFI/BIOS, inaktivera CSM/Legacy-läge och starta sedan om."
    else
        legacy_boot_message="Cordierite does not support CSM/Legacy Boot. Please boot into your UEFI/BIOS settings, disable CSM/Legacy Mode, and reboot."
...
serve_docs
   if is_swedish_locale; then
       text="Du behöver <b>minst 8 GB systemminne</b> för att installera Cordierite.\n\nInstallationen misslyckas troligen med 4 GB minne eller mindre.\n\nIdentifierad mängd minne: $gb_memory\n\nLäs <a href=\"http://127.0.0.1:1290/Gaming/Hardware_compatibility_for_gaming/#minimum-system-requirements\">dokumentationen</a> om Cordierites lägsta systemkrav."
       title="För lite minne"
       shutdown_button="Stäng av"
   else
       text="You need <b>at least 8GB of system memory</b>  to install Cordierite. \n\n Installation with 4GB or less memory will likely fail.\n\nDetected amount of memory: $gb_memory\n\n Please read <a href=\"http://127.0.0.1:1290/Gaming/Hardware_compatibility_for_gaming/#minimum-system-requirements\">here</a> about minimum system requirements for Cordierite."
...
inte representerar den installerade upplevelsen."
        ...
        install_button="Installera Cordierite"
...
nvidia_hardware_helper() {
    ...
    image_name=$(timeout $timeout_seconds sudo podman images --format '{{ index .Names 0 }}' | grep -E '(cordierite|bazzite)')
    ...
    if [[ -x "/usr/libexec/bazzite-detect-nvidia-support-status" ]]; then
        ...
        #user facing text
        title="Cordierite Hardware Helper"
        image_detected="Detected Cordierite version: $(echo "$image_name" |  cut -d '/' -f3)\n\n"
        ...
        detected_nvidia_deck="WARNING: Nvidia GPU Support in Steam Gaming mode and on HTPCs is available as a beta with known issues that <b>cannot be fixed</b> by Cordierite.\n\n"
        ...
        recommend_unknown="It is not recommended to install Cordierite as we cannot guarantee your hardware will work."
        heading_wrong_image="<b>WRONG IMAGE DETECTED</b>\n"
        detected_wrong_image="Your $support_status NVIDIA graphics card needs a different version of Cordierite.\n"
        ...
        button1="I KNOW WHAT I AM DOING. Install Cordierite Anyway:0"
        button2="Power Off:1"
        heading2="More Information"
        button3="More Information:2"
        if is_swedish_locale; then
            title="Cordierite hårdvaruhjälp"
            image_detected="Identifierad Cordierite-version: $(echo "$image_name" | cut -d '/' -f3)\n\n"
            ...
            detected_nvidia_deck="VARNING: Stöd för Nvidia-GPU:er i Steams spelläge och på HTPC:er är en beta med kända problem som Cordierite <b>inte kan åtgärda</b>.\n\n"
            ...
            recommend_unknown="Vi rekommenderar inte att du installerar Cordierite, eftersom vi inte kan garantera att din maskinvara fungerar."
            heading_wrong_image="<b>FEL AVBILDNING IDENTIFIERAD</b>\n"
            detected_wrong_image="Din Nvidia-GPU med statusen $support_status behöver en annan Cordierite-version.\n"
            ...
            button1="JAG VET VAD JAG GÖR. Installera Cordierite ändå:0"
            button2="Stäng av:1"
